...
function bindVideoCards() {
  ...
  $('#content').querySelectorAll('[data-vedit]').forEach((b) => b.addEventListener('click', (e) => {
    e.stopPropagation();
    const v = (DATA.videos || []).find((x) => x.id == b.dataset.vedit);
...
function videoActions(v) {
  ...
  if (isApprover && (v.status === 'biriktirildi' || v.status === 'qaytarildi'))
    b.push(`<button class="mini-btn blue" data-vedit="${v.id}">✏️ Tahrirlash</button>`);
  if (isApprover && (v.status === 'biriktirildi' || v.status === 'qaytarildi'))
...
function openVideoDetailModal(v) {
  ...
  `, () => {
    ...
    $('#modalBody').querySelectorAll('[data-vedit]').forEach((b) =>
      b.addEventListener('click', () => { closeModal(); openVideoModal(v); })
    );
  });
}
...
function openStudioBookingModal(presetDate, edit) {
  ...
  const typeOpts = Object.entries(shootTypes).map(([k, l]) => `<option value="${k}"${sel(k, e.shoot_type)}>${esc(l)}</option>`).join('');
  const opOpts = `<option value=""${sel('', e.operator || '')}>— operator yo'q —</option>` + operators.map((o) => `<option${sel(o, e.operator)}>${esc(o)}</option>`).join('');
  openModal(edit ? '✏️ Bronni tahrirlash' : 'Kadr Studio — bron qilish', `
    <div class="field"><label>Mijoz ismi</label><input id="sb_name" placeholder="Mijoz ismi" value="${esc(e.client_name || '')}" /></div>
    <div class="field-row">
      <div class="field"><label>Telefon</label><input id="sb_phone" placeholder="+998 90 123 45 67" value="${esc(e.phone || '')}" /></div>
    ...
    <div class="field"><label>Sana</label><input id="sb_date" type="date" value="${e.bdate || presetDate || ''}" /></div>
    <div class="field-row">
      <div class="field"><label>Boshlanish</label><input id="sb_start" type="time" value="${esc((e.start_time || '10:00').slice(0, 5))}" /></div>
...
async function openStudioFinanceModal() {
  ...
  const rows = f.months.map((mo, idx) => {
    const bk = (mo.bookings || []).map((x) => `
    ...
    <div class="fin-month">
      <button class="fm-head fm-toggle" data-idx="${idx}">
        <span><b>${esc(mo.month)}</b> <span class="muted">· ${mo.count} bron</span></span>
        <span><b>${money(mo.total)}</b> ${mo.debt > 0 ? `<span class="pill orange">qarz ${money(mo.debt)}</span>` : ''} <span class="fm-caret">▸</span></span>
      </button>
      <div class="fm-body" data-body="${idx}" hidden>
  ...
  openModal('🎥 Kadr Studio — pul hisoboti', `
    ...
    <p class="muted" style="margin-bottom:8px">📌 Oy ustiga bossangiz — ichidagi barcha bronlar, to'lov va qarzlar ochiladi.</p>
    <div class="fin-months">${rows}</div>`, () => {
    $$('.fm-toggle').forEach((btn) => btn.addEventListener('click', () => {
      const body = $(`[data-body="${btn.dataset.idx}"]`);
      if (!body) return;
...
// ---- Video biriktirish / tahrirlash modal (rahbar montajchiga biriktiradi) ----
async function openVideoModal(existing = null) {
  DATA.projects = await api('/api/projects');  // rahbar o'z loyihalari
  if (!DATA.scripts) DATA.scripts = await api('/api/scripts');
  const eds = await api('/api/editors');       // montajchilar (montaj qiluvchi rahbarlar ham, masalan Shodiya)
  ...
  openModal(editing ? 'Videoni tahrirlash' : 'Videoni montajchiga biriktirish', `
    ...
    <div class="field-row">
      <div class="field"><label>Video turi</label><select id="vf_vtype">${vtypes.map(([v, l]) => `<option value="${v}" ${cur.vtype === v ? 'selected' : ''}>${l}</option>`).join('')}</select></div>
      <div class="field"><label>Montajchi (kimga)</label><select id="vf_editor"><option value="">—</option>${edNames.map((n) => `<option ${cur.editor === n ? 'selected' : ''}>${esc(n)}</option>`).join('')}</select></div>
    </div>
    <div class="field-row">
  ...
  () => {
    $('#vf_save').addEventListener('click', async () => {
      ...
      if (res && res.error) { toast(res.error); return; }
      closeModal();
      toast(editing ? '💾 Saqlandi' : (res && res.due_at ? `🎬 Biriktirildi · muddat ${fmtDate(res.due_at)}` : '🎬 Biriktirildi'));
      render();
    });
  });
}
